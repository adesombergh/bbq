...
import { StateError, Store } from "../src/state.ts";
...
describe("Store", () => {
  test("round lifecycle with gating", () => {
    ...
    expect(() => s.setAnswer(sess.id, r.id, "r1q2", { kind: "text", text: "hi" })).toThrow(StateError);
    expect(() => s.submitRound(sess.id, r.id)).toThrow(StateError);

    s.setAnswer(sess.id, r.id, "r1q1", { kind: "recommended", text: "" });
    expect(r.answers.r1q1).toMatchObject({ kind: "recommended", optionId: "b", text: "y" });

    // change answer is allowed
    s.setAnswer(sess.id, r.id, "r1q1", { kind: "option", optionId: "a", text: "" });
    expect(r.answers.r1q1!.text).toBe("x");

    s.setAnswer(sess.id, r.id, "r1q2", { kind: "text", text: "free" });
    expect(s.isComplete(r)).toBe(true);
    s.submitRound(sess.id, r.id);
    expect(r.status).toBe("submitted");
    expect(() => s.setAnswer(sess.id, r.id, "r1q1", { kind: "option", optionId: "b", text: "" })).toThrow(/submitted/);
  ...
  test("only one open round at a time", () => {
    ...
    expect(() => s.addRound(sess.id, [q("B")])).toThrow(/still open/);
  });

  test("validates recommendedOptionId", () => {
    const s = new Store();
    const sess = s.createSession("t");
    expect(() => s.addRound(sess.id, [q("A", ["x"], "zz")])).toThrow(/not one of the options/);
  });
  ...
  test("asides coalesce while in flight and resolve", () => {
    ...
    expect(s.getAside(sess.id, a1.id)).toMatchObject({ status: "resolved", content: "hello" });
  ...
  test("waitFor resolves on change and times out otherwise", async () => {
    ...
    const timedOut = await s.waitFor(sess.id, (x) => (x.rounds[0]!.status === "submitted" ? true : undefined), 30);
    expect(timedOut).toBeUndefined();

    const p = s.waitFor(sess.id, (x) => (x.rounds[0]!.status === "submitted" ? "done" : undefined), 5_000);
    s.setAnswer(sess.id, r.id, "r1q1", { kind: "text", text: "ok" });
    s.submitRound(sess.id, r.id);
    expect(await p).toBe("done");
  });

  test("apply routes client messages and rejects unknown", () => {
    const s = new Store();
    const sess = s.createSession("t");
    const r = s.addRound(sess.id, [q("A", ["x"])]);
    s.apply(sess.id, { type: "answer", roundId: r.id, questionId: "r1q1", answer: { kind: "option", optionId: "a", text: "" } });
    expect(r.answers.r1q1!.text).toBe("x");
    s.apply(sess.id, { type: "clear_answer", roundId: r.id, questionId: "r1q1" });
    expect(r.answers.r1q1).toBeUndefined();
    expect(() => s.apply(sess.id, { type: "nope" } as never)).toThrow(StateError);
  });
});
